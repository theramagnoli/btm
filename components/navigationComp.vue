...
<template>
  ...
  >
    ...
    >
      ...
      <div class="justify-items-end gap-4 flex items-center" id="menu">
        <NuxtLink
          to="/"
          class="w-[100%] tracking-wider py-7 flex flex-col relative"
          ><div class="items-center flex">Home</div>
          <div v-if="route.path === '/'" class="selected-marker"
        /></NuxtLink>
        <NuxtLink
          to="/tours"
          class="w-[100%] tracking-wider px-2 py-7 flex flex-col relative"
          ><div class="items-center flex">Tours</div>
          <div v-if="route.path === '/tours'" class="selected-marker"
        /></NuxtLink>
        <div>
          <button
            @click="$emit('changeMode')"
            class="mt-[3px] rounded-full text-gray-900 dark:text-gray-200 hover:scale-110 opacity-75"
          >
            <svg
...
<style lang="postcss">
.selected-marker {
  @apply h-1.5 bg-gradient-to-r from-sky-500 to-emerald-700 dark:from-sky-700 dark:to-emerald-900 rounded-t-lg absolute bottom-0 left-0 right-0;
}
</style>
